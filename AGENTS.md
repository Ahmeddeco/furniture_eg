...
| Folder                 | Description                                                                                                                              |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `bot`           | Entry point for all Mastra-related code and configuration.                                                                               |
| `bot/agents`    | Define and configure your agents - their behavior, goals, and tools.                                                                     |
| `bot/workflows` | Define multi-step workflows that orchestrate agents and tools together.                                                                  |
| `bot/tools`     | Create reusable tools that your agents can call                                                                                          |
| `bot/mcp`       | (Optional) Implement custom MCP servers to share your tools with external agents                                                         |
| `bot/scorers`   | (Optional) Define scorers for evaluating agent performance over time                                                                     |
| `bot/public`    | (Optional) Contents are copied into the `.build/output` directory during the build process, making them available for serving at runtime |
...
| File                  | Description                                                                                                       |
| --------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `bot/index.ts` | Central entry point where you configure and initialize Mastra.                                                    |
...
- Register new agents, tools, workflows, and scorers in `bot/index.ts`
