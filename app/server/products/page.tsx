...
import {getAllProducts} from "@/dl/product.data"
...
export default async function ProductsPage({
	...
	>
		{!products?.data.length ? (<EmptyCard href={"/server/products/add"} linkTitle={"add product"}/>) : (<Table>
			{/* ---------------------------- TableHeader ---------------------------- */}
			<TableHeader>
				<TableRow>
			...
			{/* ----------------------------- TableBody ----------------------------- */}
			<TableBody>
				{products.data.map(({id, mainImage, title, model, price, status}) => (<TableRow key={id}>
					<TableCell className="relative rounded-lg size-16 ">
						{mainImage ? (<Image src={mainImage} alt={title} fill className="rounded-lg object-contain"/>) : (
							<ImageOff size={72}/>)}
					</TableCell>
			...
			<TableCaption>
				<Pagination>
					<PaginationContent>
						...
						{/* ------------------------- PaginationLink ------------------------ */}
						{Array.from({length: products.totalPages ?? 1}).map((_, index) => (<PaginationItem key={index}>
