...
export default async function TheMostFavorite() {
	...
	return (
		<section className="container mx-auto mt-8 flex flex-col lg:flex-row lg:gap-12 gap-2 items-center justify-center w-full">
			<div className="w-full lg:w-1/2 aspect-video relative rounded-2xl">
				{product?.mainImage ? (
					<Image src={product?.mainImage} alt={product?.title ?? "product"} fill className="object-cover rounded-2xl" />
				) : (
					<ImageOff />
				)}
			</div>
